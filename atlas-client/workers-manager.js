...
export class WorkersManager {
    constructor(server_count,
        ...
        // faster map hold this object
        // {
        //   remote_start // remote start time
        //   remote_end   // remote start time
        //   local_start  // to run locally
        //   local_end    // to run locally
        // }
        this.fasters     = new Map();
    ...
    update_onmessage_resolvers(worker) {
        worker.onmessage = (e) =>  {
            ...
            // console.log(`workers-manager.js:68 e.data.values.fulfill=${e.data.values.fulfill}`);
...
//             const worker_result = eval('(' + e.data.values.data + ')');
            if (!this.completed_locally.has(e.data.values.req_id)) {
                ...
                if(e.data.values.fulfill === 'SUCCESS') {
                    // console.log(`workers-manager.js:77 e.data.values.fulfill=${e.data.values.fulfill}`);
                    this.resolvers.get(e.data.values.req_id)(worker_result);
                    var faster = this.fasters.get(e.data.values.func);
                    if(faster !== undefined) {
                        // console.log(`workers-manager.js:82 faster.count=${faster.count}`);
                        faster.remote_end = os.now();
//                        ++faster.count;
                    }
                }
