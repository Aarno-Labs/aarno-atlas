...
export class AtlasWrapper {
    constructor(workerManager) {
        ...
        //         var faster = this.workerManager.faster_locals.get(target);
        //         if(local_execution
        //            || (faster && !faster.tested_local && !faster.test_local)) {
        //             faster.tested_local = true;
    ...
    handler_func(offload) {
        return {
            apply: (target, p1, ...arguments_list) => {
                // console.log(`atlas-wrapper.js:54`);
                // iterate all wrapped objects
                let target_name = get_real_target_name(target);
                let faster_local = false;

                // console.log(`atlas-wrapper.js:128: offload.only_faster=${offload.only_faster}`);
                if(offload.only_faster !== undefined) {
                    // console.log(`atlas-wrapper.js:130 faster target=${target_name}`);
                    let faster = this.workerManager.fasters.get(target_name);
                    // console.log(`atlas-wrapper.js:132 ${faster}`);
                    if(faster === undefined) {
                        faster = { remote_start : offload.remote_start * 0 || -1,
                                   remote_end   : offload.remote_start * 1000 || -1,
                                   local_start  : offload.local_start * 0 || -1,
                                   local_end    : offload.local_end * 0 || -1,
                                   count        : 0
                                 };
                        // console.log(`atlas-wrapper.js:139`);
                        this.workerManager.fasters.set(target_name, faster);
                        // console.log(`atlas-wrapper.js:140`);
                    }
                    const need_first_remote = faster.remote_start == -1;
                    const need_first_local  = faster.local_start == -1;
                    // console.log(`remote ${faster.remote_end} - ${faster.remote_start} > ${faster.local_end} - ${faster.local_start} `);
                    // console.log(`remote ${faster.remote_end - faster.remote_start} > ${faster.local_end - faster.local_start} `);
                    const local_faster = faster.remote_end - faster.remote_start > faster.local_end - faster.local_start;
                    faster_local = !need_first_remote && (need_first_local || local_faster);
                    // console.log(`faster_local ${faster_local} faster.remote_start=${faster.remote_start} need_first_remote=${need_first_remote}`);
                    // console.log(`switch on count faster.count=${faster.count} == offload.only_faster.check_count=${offload.only_faster.check_count} ${faster_local}`);
                    if(faster.count == offload.only_faster.check_count) {
                        // console.log(`switch on count faster.count=${faster.count} == offload.only_faster.count=${offload.only_faster.check_count} ${faster_local}`);
                        faster.count = 0;
                        faster_local = !faster_local;
                    }
                }

                if(local_execution || faster_local) {
                    // console.log('atlas_wrapper.js:145 local run');
                    if(offload.only_faster) {
                        // console.log(`atlas-wrapper.js:155 target=${target_name}`);
                        let faster = this.workerManager.fasters.get(target_name);
                        faster.local_start = os.now();
                    }
                    this.atlas_nested_fcalls++;
                    console.log("[" + getDateTime() + "] LOCAL");
                    
                    var e = exec_local(target, target_name, ...arguments_list, "exec");
                    if(offload.only_faster) {
                        // console.log(`atlas-wrapper.js:163 target=${target_name}`);
                        let faster = this.workerManager.fasters.get(target_name);
                        faster.local_end = os.now();
                        ++faster.count;
                    }                    
                    this.atlas_nested_fcalls--;
                    return e;
                ...
                try {
                    console.log("[" + getDateTime() + "] REMOTE");
        ...
        return new Promise((resolve, reject) => {
            ...
            this.workerManager.local_exec.set(request.req_id,
                                              { target : target,
                                                ...
                                                start_time: atlas_tools.get_time()});
            
            // console.log(`atlas-wrapper.js:222 function_name=${function_name}`);
            if(!this.workerManager.fasters.has(function_name)) {
                this.workerManager.fasters.set(function_name,
                                               { remote_start : -1,
                                                 local_start  : -1,
                                                 local_end    : -1});
            }
            // console.log(`atlas-wrapper.js:249 function_name=${function_name}`);
            var faster = this.workerManager.fasters.get(function_name);
            if(faster !== undefined) {
                // console.log(`atlas-wrapper.js:251 function_name=${function_name}`);
                faster.remote_start = os.now();
                // console.log(`atlas-wrapper.js:253 function_name=${function_name}`);
            }
