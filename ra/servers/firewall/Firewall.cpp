...
Message * Firewall::handlePassportCheck(DeviceManager &deviceManager, PassportCheck *passportCheck, Device *device)
{
    ...
    if (!ok) {
      ...
      permission->setAdmittance(DENIED);
      permission->setCause(CAUSE_INVALID_PASSPORT);
...
void Firewall::carbonCopy(Endpoint &endpoint, Message *message)
{
    ...
    std::unique_ptr<EndpointSock> eps;
    while (1) {
        sock = Comm::connectTcp((Endpoint *) &endpoint);
        eps.reset(FinishClientConnection(sock, endpoint));
        if (sock > 0)
            break;
        i++;
        if (i >= MAX_ATTEMPTS) {
    ...
    SD_LOG(LOG_DEBUG, "%s:%d %s sock=%d", __FILE__, __LINE__, __FUNCTION__, sock);
    finalizeAndSend(*eps, message, *this, "Firewall::carbonCopy");

    char buf[32];
    // recv(sock, buf, 32, 0); // ack; content irrelevant
    eps->recv_data(sock, buf, 32);
    //    close(sock);
    close(sock);
}
...
Acceptance Firewall::forward(Data *data, Config &config, uint8_t *serialized, uint32_t len)
{
    ...
    if (accept != ACCEPT) {
        ...
        if (sock < 0) {
            ...
            auto eps = EndpointSock{*appSvrEndpoint, sock};
            bool sent = sendMessage(eps, DATA, serialized, len, "Firewall::forward");
            if (sent) {
                accept = ACCEPT;
            }
            else {
...
void Firewall::forward(KeyChange *keyChange, uint8_t *serialized, uint32_t len)
{
    ...
    if (map == actions.end() || map->second != CORRECT) {
        ...
        if (sock < 0) {
            ...
            auto eps = EndpointSock{*appSvrEndpoint, sock};
            bool sent = sendMessage(eps, KEY_CHANGE, serialized, len, "Firewall::forward");
            if (sent) {
