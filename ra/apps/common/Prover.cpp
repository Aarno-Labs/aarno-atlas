...
void Prover::run()
{
    ...
    while (true) {
        ...
        int mySock = Comm::connectTcp(ep);
        
        if (mySock < 0) {
            toGiveup(false, &proc_fail_count, true);
        }
        else {
          SD_LOG(LOG_DEBUG, "[%s:%d]Prover Connect %s", __FILE__, __LINE__, ep->toString().c_str());
          myEndPointSock.reset(FinishClientConnection(mySock, *ep));
          //          myEndPointSock = EndpointSock(*ep, mySock);
            proc_completed = true;
            runProcedure(*myEndPointSock);
            toGiveup(proc_completed, &proc_fail_count, true);

            // close(mySock);
            SD_LOG(LOG_DEBUG, "closed socket");
        }
        if(this->passport_granted_) {
          break;
        }
...
void Prover::runProcedure(EndpointSock &eps)
{
    ...
    while (true) {
        if (this->passport_granted_) {
          SD_LOG(LOG_DEBUG, "passport granted");
          break;
        }
        // int bytesRead = recv(eps.getSock(), ptr, avail, 0);
        const int bytesRead = eps.recv_data(eps.getSock(), ptr, avail);
        if (bytesRead <= 0) {
...
bool Prover::moveTo(MessageID id, Message *received)
{
    ...
    case ATTESTATION_REQUEST:
        ...
        break;
    case EVIDENCE:
      to_send   = prepareEvidence(received, attestSqn, config.getComponent().getID(), *board, sediment_home, seec);
        expecting = GRANT;
        break;
    case PASSPORT_CHECK:
    ...
    if (to_send != NULL) {
      SD_LOG(LOG_DEBUG, "[deviceID=%s]", config.getComponent().getID().c_str());
        to_send->setDeviceID(config.getComponent().getID());
        finalizeAndSend(*myEndPointSock, to_send, *this, "Prover::moveTo");
        delete to_send;
    }
    return towait;
}
                                  ...
                                  Seec &seec)
{
    ...
    for (int i = 0; i < numEvidenceType; i++) {
        ...
        case EVIDENCE_FULL_FIRMWARE:
          itemOk = prepareEvidenceFullFirmware(challenge, &items[count], &elapsed, &optional, board, seec);
            count++;
            break;
        case EVIDENCE_OS_VERSION:
          itemOk = preapreEvidenceOsVersion(challenge, &items[count], board, seec);
            count++;
            break;
        case EVIDENCE_BOOT_TIME:
            itemOk = preapreEvidenceBootTime(challenge, &items[count], board);
            count++;
            break;
        case EVIDENCE_CONFIGS:
          itemOk = prepareEvidenceConfigs(challenge, &items[count], &elapsed, &optional, board, seec);
            count++;       
            break;
        case EVIDENCE_UDF_LIB:
#ifdef PLATFORM_RPI        
          itemOk = prepareEvidenceUDFLib(challenge, &items[count], &elapsed, &optional, sediment_home, board, seec);
            count++;
#else
        ...
        case EVIDENCE_UDF3:
#ifdef PLATFORM_RPI
          itemOk = preapreEvidenceUDF(challenge, &items[count], type, sediment_home, seec);
            count++;
#else
    ...
    evidence->setDeviceID(component_id);
...
bool Prover::handleGrant(Message *received)
{
    ...
    proc_completed   = true;
    full_reset_count = 0;
    passport_granted_ = true;
    SD_LOG(LOG_DEBUG, "handleGrant done");
...
bool Prover::preapreEvidenceBootTime(Challenge *challenge,
                                     EvidenceItem *item,
                                     Board &board)
{
    (void) challenge;
...
bool Prover::preapreEvidenceUDF(Challenge *challenge,
                                EvidenceItem *item,
                                   ...
                                   const std::string &sediment_home, Board &board, Seec &seec)
{
    ...
    return prepareEvidenceHashing(challenge, item, elapsed, optional, EVIDENCE_UDF_LIB, starting, blockSize, seec, board);
}

#endif // ifdef PLATFORM_RPI

bool Prover::preapreEvidenceOsVersion(Challenge *challenge, EvidenceItem *item, Board &board, Seec &seec)
{
    (void) challenge;
...
bool Prover::prepareEvidenceFullFirmware(Challenge *challenge, EvidenceItem *item, uint32_t *elapsed, int *optional, Board &board, Seec &seec)
{
    uint32_t blockSize      = challenge->getBlockSize();
    string lib              = "sediment";
    const uint8_t *starting = (const uint8_t *) board.getStartingAddr(lib, &blockSize);

    SD_LOG(LOG_INFO, "attest firmware starting address: 0x%0x %u", starting, blockSize);

    return prepareEvidenceHashing(challenge, item, elapsed, optional, EVIDENCE_FULL_FIRMWARE, starting, blockSize, seec, board);
}

bool Prover::prepareEvidenceConfigs(Challenge *challenge, EvidenceItem *item, uint32_t *elapsed, int *optional, Board &board, Seec &seec)
{
    uint32_t blockSize;
    char *starting = board.getConfigBlocks((int *)&blockSize);   // memoery allocation

    bool val = prepareEvidenceHashing(challenge, item, elapsed, optional, EVIDENCE_CONFIGS, (const uint8_t *)starting, blockSize, seec, board);
    free(starting);
