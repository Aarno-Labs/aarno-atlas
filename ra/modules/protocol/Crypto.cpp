...
int Crypto::init()
{
  SD_LOG(LOG_INFO, "[%s:%d %s()] authKey=%s", __FILE__, __LINE__, __FUNCTION__, authKey);
    return 0;
}
...
int Crypto::checksum(KeyPurpose keyPurpose, Block *blocks, int block_count, uint8_t *digest, int digest_size)
{
    ...
    int key_size = getKeySize(keyPurpose);
      ...
      SD_LOG(LOG_INFO, "[%s:%d %s()] keyPurpose=%d key=%s", __FILE__, __LINE__, __FUNCTION__,
             keyPurpose,
             Log::toHexNoLimit((char *) key ,key_size).c_str()
             );
  ...
  int digest_size)
{
      SD_LOG(LOG_INFO, "[%s:%d %s()] keyPurpose=%d key=%s", __FILE__, __LINE__, __FUNCTION__,
             keyPurpose,
             Log::toHexNoLimit((char *) key, key_size/8).c_str()
             );
    (void) digest_size;
...
int Crypto::init()
{
    SD_LOG(LOG_DEBUG, "[%s:%d %s()] authKey=%s", __FILE__, __LINE__, __FUNCTION__, authKey);
    return 0;
}
...
int Crypto::checksum(KeyPurpose keyPurpose, Block *blocks, int block_count, uint8_t *digest, int digest_size)
{
    ...
    int key_size = getKeySize(keyPurpose);
      ...
      SD_LOG(LOG_DEBUG, "[%s:%d %s()] keyPurpose=%d key=%s", __FILE__, __LINE__, __FUNCTION__,
             keyPurpose,
             Log::toHexNoLimit((char *) key ,key_size / 8).c_str()
             );
      SD_LOG(LOG_DEBUG, "[%s:%d %s()] key_str==%s", __FILE__, __LINE__, __FUNCTION__, (const char *)key);
    return checksum(key, key_size, blocks, block_count, digest, digest_size);
}
...
int Crypto::checksum(const unsigned char *key, int key_size, Block *blocks, int block_count, uint8_t *digest,
  int digest_size)
{
    SD_LOG(LOG_DEBUG, "[%s:%d %s()] keySize=%d key=%s", __FILE__, __LINE__, __FUNCTION__,
         key_size,
         Log::toHexNoLimit((char *) key ,key_size / 8).c_str()
         );
    SD_LOG(LOG_DEBUG, "[%s:%d %s()] key_str==%s", __FILE__, __LINE__, __FUNCTION__, key);
